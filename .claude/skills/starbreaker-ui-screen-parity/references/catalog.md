...
| "This render looks identical to the last — my change did nothing" | `ui_arc_status.sh`/`ui_render.sh` write a UNIQUE path per run, but a viewer can still cache by name — confirm an on-disk change via the printed `png md5:` line before concluding no-op; never re-tune blind on a "looks identical" report (ledger 69). |
